...
def kp_detection(db, k_ind, data_aug, debug):
    ...
    # 肯定是需要使用list来存储的，因为不同图像中的框的数量是不一致的
    gt_bboxes = []
    gt_labels = []
    ...
    for b_ind in range(batch_size):
        ...
        if detections is not None:
            gt_bboxes.append(
                torch.tensor(detections[:, :-1], dtype=torch.float32))
            gt_labels.append(
                torch.tensor(detections[:, -1] - 1, dtype=torch.float32))
        else:
            gt_bboxes.append(torch.zeros((1, 4), dtype=torch.float32))
            gt_labels.append(torch.zeros((1, ), dtype=torch.float32))
        #print('gt_bboxes:',gt_bboxes.shape)
        #print('sample coco detections:', detections.shape)
        #pdb.set_trace()
    ...
    #print('sample coco detections:', detections)
    #print('sample coco ind:',ind)
    for b_ind in range(batch_size):
    ...
    #gt_bboxes = [torch.from_numpy(bbox) for bbox in gt_bboxes]
    #gt_labels = [torch.from_numpy(label) for label in gt_labels]
...
def sample_data(db, k_ind, data_aug=True, debug=False):
    return globals()[system_configs.sampling_function](db, k_ind, data_aug,
                                                       debug)
