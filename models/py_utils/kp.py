...
from .kp_utils import _sigmoid, _ae_loss, _regr_loss, _neg_loss, _centerness_loss
...
from ..Centerness import CenterNessNet
...
class kp(nn.Module):
    def __init__(self,
        ...
        for tl_heat, br_heat, ct_heat in zip(self.tl_heats, self.br_heats,
                                             self.ct_heats):
        ...
        self.CenterNessNet = CenterNessNet(in_channels=256,
                                           feat_channels=256,
                                           stacked_convs=4,
    ...
    def _train(self, *xs):
        ...
        layers = zip(self.kps, self.cnvs, self.tl_cnvs, self.br_cnvs,
                     self.ct_cnvs, self.tl_heats, self.br_heats, self.ct_heats,
                     self.tl_tags, self.br_tags, self.tl_regrs, self.br_regrs,
                     self.ct_regrs)
        for ind, layer in enumerate(layers):
            ...
            tl_heat, br_heat, ct_heat = tl_heat_(tl_cnv), br_heat_(
                br_cnv), ct_heat_(ct_cnv)
            tl_tag, br_tag = tl_tag_(tl_cnv), br_tag_(br_cnv)
            tl_regr, br_regr, ct_regr = tl_regr_(tl_cnv), br_regr_(
                br_cnv), ct_regr_(ct_cnv)
            ...
            outs += [
                tl_heat, br_heat, ct_heat, tl_tag, br_tag, tl_regr, br_regr,
                ct_regr
            ]
            ...
            if (ind >= self.nstack - 1):
                # error
                centerness_map = self.CenterNessNet(cnv)
                outs += [centerness_map]

        return outs

    def _test(self, *xs, **kwargs):
        #print('py_utils kp xs:', xs)
        image = xs[0]
...
class CenterNessLoss(nn.Module):
    def __init__(self, centerness_weight=1):
        super(CenterNessLoss, self).__init__()
        self.centerness_weight = centerness_weight
        self.centerness_loss = _centerness_loss

    def forward(self, outs, gt_bboxes=None, gt_labels=None):
        centerness_loss = self.centerness_weight * self.centerness_loss(
            outs, gt_bboxes, gt_labels)
        centerness_loss = centerness_loss / len(outs)
        return centerness_loss.unsqueeze(0)
...
class AELoss(nn.Module):
    ...
    def forward(self, outs, targets):
        stride = 8
        #print('py_utils outs len:', len(outs))
        #print('py_utils targets len:', len(targets))
        #print('py_utils gt_bboxess len:', len(gt_bboxess))
        #print('py_utils gt_labelss len:', len(gt_labelss))
        ...
        return loss.unsqueeze(0), (focal_loss / len(tl_heats)).unsqueeze(0), (
            pull_loss / len(tl_heats)).unsqueeze(0), (
                push_loss /
                len(tl_heats)).unsqueeze(0), (regr_loss /
                                              len(tl_heats)).unsqueeze(0)
