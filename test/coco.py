    ...
    for ind in tqdm(range(0, num_images), ncols=80, desc="locating kps"):
        ...
        if len(scores) > max_per_image:
            ...
            for j in range(1, categories + 1):
                ...
                #top_bboxes[image_id][j] = select_box(top_bboxes[image_id][j])
        ...
        for i in range(length):
            if i == max_index:
                ...
                if (int(boxes[i][0]) >= int(boxes[max_index][0])
                        ...
                        and boxes[i][4] > boxes[max_index][4]
                        and max_area > 3 * areas[i]):
...
#                        and boxes[i][4] > boxes[j][4]):
#                    boxes[j][4] = -1
#                    print('detele a num')
#                elif (boxes[i][0] <= boxes[j][0] and boxes[i][1] <= boxes[j][1]
#                      and boxes[i][2] >= boxes[j][2]
#                      and boxes[i][3] >= boxes[j][3]
#                      and boxes[i][4] < boxes[j][4]):
#                    boxes[i][4] = -1
...
#                else:
#                    continue
#    keep_inds_1 = boxes[:, -1] > 0
#    boxes = boxes[keep_inds_1]
#
#    return boxes
