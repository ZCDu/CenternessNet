...
def pin_memory(data_queue, pinned_data_queue, sema):
    while True:
        ...
        data["gt_bboxes"] = [box.pin_memory() for box in data["gt_bboxes"]]
        data["gt_labels"] = [label.pin_memory() for label in data["gt_labels"]]
        #data = []
        #for _, y in enumerate(data["ys"]):
        #    if isinstance(y, list):
        #        children = []
        #        for _, i in enumerate(y):
        #            children.append(i.pin_memory())
        #        data.append(children)
        #    else:
        #        data.append(y.pin_memory())
        #data["ys"] = data
...
def init_parallel_jobs(dbs, queue, fn, data_aug):
    tasks = [
        Process(target=prefetch_data, args=(db, queue, fn, data_aug))
        for db in dbs
    ]
    for task in tasks:
        task.daemon = True
...
def train(training_dbs, validation_db, start_iter=0):
    ...
    with stdout_to_tqdm() as save_stdout:
        for iteration in tqdm(range(start_iter + 1, max_iteration + 1),
            ...
            training_loss, focal_loss, pull_loss, push_loss, regr_loss, centerness_loss = nnet.train(
                iteration, **training)
            ...
            if display and iteration % display == 0:
                ...
                print("centerness loss at iteration {}:      {}\n".format(
                    iteration, centerness_loss.item()))
